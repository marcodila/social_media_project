...
def friends(request):
    ...
    admin_profile, _ = Profile.objects.get_or_create(user_id=1)
    ...
    #to get eligible profiles - exclude the user, their friends, amd requests they sent already
    ...
    request_received_profiles = Relationship.objects.filter(receiver=user_profile,status='send')

    if not user_relationships.exists():
        Relationship.objects.create(sender=user_profile,receiver=admin_profile,status='send')
    ...
    if request.method == 'POST' and request.POST.get("send_requests"):
        ...
        for receiver in receivers:
            ...
            Relationship.objects.create(sender=user_profile,receiver=receiver_profile,status='send')
